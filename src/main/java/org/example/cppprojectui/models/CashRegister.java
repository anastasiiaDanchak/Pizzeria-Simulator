...
import org.example.cppprojectui.data.FirebaseInitializer;
import org.example.cppprojectui.data.FirebaseOrderService;
...
public class CashRegister implements Runnable {
    ...
    private void processClient(Client client) {
        ...
        for(Pizza pizza : order.getPizzas()) {
            ...
            order.getState();
            OrderDTO updatedOrderDTO = order.toDTO();
            FirebaseOrderService.getInstance().updateOrder(this.id,client.getOrderDTO().getOrderUUID(),updatedOrderDTO);
        }
        isProccesing = false;
